...
Пользовательские действия не создают дополнительные `simulation:tick`. Frontend накапливает запуск и сброс incidents, управление устройствами, `human:move` и вызванные движением события датчиков, а затем прикладывает их к следующему плановому tick. Только периодический таймер отправляет `simulation:tick`, поэтому частота действий пользователя не изменяет `simTime` и скорость распространения incidents.
