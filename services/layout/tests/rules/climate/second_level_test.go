...
func TestClimateLevel2(t *testing.T) {
	apartmentStruct := &apartment.Apartment{
		...
		Rooms: []apartment.Room{
			{ID: "r1", Name: apartment.RoomLiving, Area: []point.Point{{X: 0, Y: 0}, {X: 4, Y: 0}, {X: 4, Y: 4}, {X: 0, Y: 4}}, Walls: []string{"w1", "w2", "w3", "w4"}},
	...
	globalPlacement, err := e.PlaceDevices(apartmentStruct, map[string]string{"climate": "2"})
	assert.NoError(t, err)

	for _, roomID := range []string{"r1", "r2", "r3", "r4"} {
		assert.True(t, globalPlacement.HasDeviceInRoom("temperature_sensor", roomID))
		assert.True(t, globalPlacement.HasDeviceInRoom("smart_radiator_actuator", roomID))
	}
	for _, roomID := range []string{"r1", "r2", "r3"} {
		assert.True(t, globalPlacement.HasDeviceInRoom("humidity_sensor", roomID))
	}
	for _, roomID := range []string{"r1", "r2"} {
		assert.True(t, globalPlacement.HasDeviceInRoom("smart_humidifier", roomID))
	}
}
