...
func TestSimpleThirdLevelScript(t *testing.T) {
	...
	for _, devicePlacement := range globalPlacement.Placements[room.ID] {
		...
		case "smart_tv":
			...
			assert.Equal(t, &point.Point{X: 4500, Y: 7000.0}, devicePlacement.Position)
		case "smart_speaker":
			foundSpeaker = true
		...
		case "subwoofer":
			...
			isLeftCorner := devicePlacement.Position.X < 1000.0
			isRightCorner := devicePlacement.Position.X > 8000.0

			assert.True(t, isLeftCorner || isRightCorner)
			assert.InDelta(t, 7000.0, devicePlacement.Position.Y, 1000.0)
		}
	}
