...
При размещении пожара frontend одновременно активирует `fire` с kind `fire:spread` и `smoke` с kind `smoke:spread`. Затопление активирует `flood` с kind `flood:spread`. После активации frontend не отправляет incident повторно на каждом tick: backend сам выполняет следующий BFS-шаг при последующих `simulation:tick`.
...
Тест проверяет `401` без токена и с неверным токеном, затем запускает simulation с валидным JWT, размещает пожар, ожидает backend polygons и датчик дыма, выполняет reset и повторную активацию.
