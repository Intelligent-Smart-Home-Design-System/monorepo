COMPOSE_PIPELINE := docker compose -f docker-compose.yml
...
pipeline-logs: ## Логи пайплайна
	$(COMPOSE_PIPELINE) logs -f pipeline-worker temporal catalog-postgresql grafana
...
up-all: pipeline-up app-up ## Запустить все стеки

down-all: pipeline-down app-down ## Остановить все стеки
