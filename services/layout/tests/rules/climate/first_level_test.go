...
func TestClimateLevel1(t *testing.T) {
	...
	globalPlacement, err := e.PlaceDevices(apartmentStruct, map[string]string{"climate": "1"})
	...
	assert.False(t, globalPlacement.HasDeviceInRoom("temperature_sensor", "r5"))
	assert.False(t, globalPlacement.HasDeviceInRoom("smart_radiator_actuator", "r5"))
}
