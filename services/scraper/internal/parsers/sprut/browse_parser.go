...
func (p *DiscoveryParser) Source() string { return domain.SourceSprut }
...
func (p *DiscoveryParser) Parse(pageSnapshotID int, files []*parser.ArchiveFile) (*BrowseLinks, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s snapshot %d: %w", domain.SourceSprut, pageSnapshotID, err)
	}
...
func (p *CategoryParser) Source() string { return domain.SourceSprut }
