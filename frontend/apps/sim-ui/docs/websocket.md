...
`payload.kind` содержит тип команды или события. Для совместимости UI также может временно дублировать `kind` на верхнем уровне события.
...
* `entityId` — сущность, которая совершает действие.
* `trigger` — ID устройства, которое было триггернуто человеком или другим событием.
